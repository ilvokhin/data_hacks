...
def run(input_stream, options):
    ...
    print("# each " + options.dot +
          " represents a count of %d. total %d" % (scale, total))
    ...
    if options.sort_values:
        ...
        if options.numeric_sort:
            ...
            data.sort(key=lambda x: (
                Decimal(x[1])), reverse=options.reverse_sort)
        else:
    ...
    for value, key in data:
        ...
        print(str_format % (key[:max_length], value,
              (value // scale) * options.dot, percentage))
